...
fcopy (int in_fd, int out_fd, size_t gap, bool stop_if_input_unlocked)
{
	...
	posix_fadvise (in_fd, (off_t) 0, (off_t) 0,
...
shake_reg_backup_phase (struct accused *a, struct law *l)
{
	...
	const int           res =
		fcopy (a->fd, l->tmpfd, MAGICLEAP, l->locks);
	posix_fadvise (l->tmpfd, (off_t) 0, (off_t) 0,
				   POSIX_FADV_WILLNEED);
...
shake_reg_rewrite_phase (struct accused *a, struct law *l)
{
	...
	if (0 > fcopy (l->tmpfd, a->fd, GAP, false))
		error (1, errno,
			   "%s: restore failed ! file have been saved at %s",
			   a->name, l->tmpname);
...
list_dir (char *name, int sort)
{
	...
	for (n = 0; (ent = readdir (dir)); n++) {
		...
		{
			char              **nbuff =
				realloc (buff, (n + 2 + BUFFSTEP) * sizeof (*buff));
			if (!nbuff) {
